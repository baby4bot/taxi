...
## 10. ✅ ล็อกอิน / สมัครด้วย Google — **ทำในแอปแล้ว** (16 ก.ย. 2569)

ต่างจากหัวข้อ 1–9 ตรงนี้คือ **ของที่ทำเสร็จและอยู่ในไฟล์ `index.html` จริง** เหลือแค่เปิดสวิตช์ในคอนโซล

### ทำอะไรได้แล้ว

| จุดในแอป | พฤติกรรม |
|---|---|
| หน้าเข้าสู่ระบบ → ปุ่ม **“ดำเนินการต่อด้วย Google”** | ครั้งแรก **สร้างบัญชีให้อัตโนมัติ** (สิทธิ์ `User` · taxiType 35 · ธีมดำ OLED) แล้วเข้าสู่ระบบให้เลย |
| ล็อกอินซ้ำด้วย Google เดิม | เข้า **บัญชีเดิม** ทันที (ไม่สร้างใหม่) |
| หน้าโปรไฟล์ → การ์ด **“บัญชี Google (ทางเลือกล็อกอิน)”** | บัญชีเก่า (username/รหัสผ่าน) กด **“ผูกบัญชี Google”** ได้ · ผูกแล้วมีปุ่ม **“ยกเลิกการผูก”** |
| บัญชีที่ถูกระงับ | บล็อกด้วยข้อความ “บัญชีของคุณถูกระงับการใช้งาน” |
| รหัสผ่านเดิม | **ยังใช้ได้เสมือนเดิมทุกประการ** (ไม่แตะ) — Google เป็นทางเลือกเพิ่ม |

### เปิดใช้ในคอนโซล (~5 นาที · ทำครั้งเดียว)

1. https://console.firebase.google.com → โปรเจกต์ **mytalkie-3955a**
2. **Authentication → Get started** (ถ้ายังไม่เคยเปิด) → แท็บ **Sign-in method** → **Add new provider** → **Google**
3. เปิด **Enable** · ใส่ *Project public-facing name* (เช่น `Taxi Fare`) · **Support email** (อีเมลคุณ)
4. **Save**
5. **Authentication → Settings → Authorized domains** → ต้องมี `baby4bot.github.io` (ปกติมีอยู่แล้ว) · โดเมน `localhost` มีให้อัตโนมัติ

> 🔎 ตรวจว่าเสร็จหรือยัง: เปิดแอป → กดปุ่ม Google → ถ้ายังไม่เปิด provider จะขึ้นข้อความไทยว่า
> *“ยังเปิด Google ไม่เสร็จ — ต้องไปเปิดที่ Firebase Console → Authentication → Sign-in method → Google”*

### กลไกเบื้องหลัง (อ่านก่อนแก้โค้ด)

- ใช้ SDK แบบ **dynamic import** `firebase-auth.js@10.8.0` เฉพาะตอนกดปุ่ม (ไม่ถ่วงเวลาเปิดแอป)
- **uid ของ Google → เก็บบันทึกการผูกไว้ที่เอกสาร `authLink/{uid}` = `{ docId, email, linkedAt }`** → ทุกอย่างหลังจากนั้นเดินโฟลว์เดิม (`users/{docId}` · `listenToMyUser()` · PIN · สิทธิ์ · บันทึกการเข้าใช้)
- ฝั่งแอปเก็บ `users/{docId}.authProvider = 'google'` · `authLinked = true` · `password = ''`
  ⇒ บัญชีที่สร้างด้วย Google **ล็อกอินด้วยรหัสผ่านไม่ได้** (ต้องกด Google หรือตั้ง PIN/รหัสผ่านทีหลัง)
- `signInWithPopup` เป็นหลัก · ถ้าเบราว์เซอร์บล็อกจะถอยไป `signInWithRedirect` เอง
- **แอปในเบราว์เซอร์ LINE/Facebook (webview) จะล็อกอิน Google ไม่ได้** — โค้ดตรวจ UA แล้วเตือนให้เปิดใน Chrome/Safari
- คำขอทั้งหมดนี้ **ไม่กินโควตา Firestore เพิ่ม** (authLink = อ่าน 1 · เขียน 1 ต่อการผูกหนึ่งครั้ง)

### กับดักที่ห้ามลืม

| อาการ | สาเหตุ | ทำอะไร |
|---|---|---|
| กดปุ่มแล้วขึ้น “ยังเปิด Google ไม่เสร็จ” | ยังไม่ Enable provider | ข้อ 2–4 ด้านบน |
| กดแล้วขึ้น “โดเมนนี้ยังไม่ได้รับอนุญาต” | โดเมนไม่อยู่ใน Authorized domains | เพิ่ม `baby4bot.github.io` |
| กดใน LINE แล้วเงียบ | Google บล็อก webview | เปิดใน Chrome/Safari |
| ล็อกอิน Google แล้วเจอคนละบัญชี | คนละอีเมล Google | ใช้ปุ่ม **“ผูกบัญชี Google”** ในโปรไฟล์เพื่อผูกให้ตรงบัญชี |
