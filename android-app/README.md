...
1. ~~**ใช้พิกัดจากคิวคำนวณ “เวลารถติดจริง”** แทนค่าประมาณ~~ — **ทำแล้ว 18 ก.ย. 69**:
   หน้าเว็บอ่านพิกัดที่เบิกมา (`window.nativeFixes`) ผ่าน `window.__gapNativeEvidence()` แล้วใช้ตัดสิน
   ระยะ/เวลารถติดช่วงจอดับ (ถอยไปใช้วิธีเดิมเมื่อหลักฐานไม่พอ) · ทดสอบด้วย `tests/native-fixes-offline.html` (10/10)
   · ต้องมี APK รุ่นที่มีบริการฉากหลัง + ขอยกเว้นประหยัดแบตเตอรี่ จึงจะเก็บพิกัดได้ครบช่วง
2. ปุ่มในแอป: “ตรวจสิทธิ์ · ดูจำนวนพิกัดที่เก็บได้”
3. ~~Native Google Sign-In~~ — **ทำแล้ว** (ดูหัวข้อ Google ใน `../.freebuff/run.md` · เหลือแต่ทะเบียน SHA-1 ฝั่งคอนโซล)
4. ~~อัปเดตอัตโนมัติ~~ — **ทำแล้ว**: แอปเช็ค `version.json` เทียบ `APP_WEB_VERSION` แล้วรีโหลดเอง (ดู `../.freebuff/run.md`)
   · โค้ดชั้น Android เปลี่ยน = ต้องออก APK ใหม่ · แก้แต่หน้าเว็บ = ไม่ต้อง
